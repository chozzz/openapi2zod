...
export function openApiSchemaToZod(
  ...
  if ('not' in schema && schema.not) {
    let combinedSchema = z.any().refine((value) => !openApiSchemaToZod(schema.not, schemas).safeParse(value).success);
    ...
    switch (schema.type) {
            ...
            case "hostname":
              stringSchema = stringSchema.refine(
                (val) => {
                  ...
                  const hostnameRegex = /^(?=.{1,253}$)(?!\-)(?:[a-zA-Z0-9\-]{0,62}[a-zA-Z0-9]\.)+[a-zA-Z]{2,63}$/;
      ...
      case "object": {
        ...
        for (const [key, value] of Object.entries(properties)) {
          let propertySchema = openApiSchemaToZod(value, schemas);
